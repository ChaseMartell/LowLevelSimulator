...
class View:
    ...
    def createWindow(self):
        ...
        # Displays the greeting to the simulator
        greeting = tk.Label(
            self.main_frame,
            text="---- Instructions given in the simulator must be in the format of +0000.  Ex. +1001 is a valid instruction ----",
        )
    ...
    def helpButton(self):
        """ Suplies an overview of the simulator"""
        messagebox.showinfo(
            "?",
            f"""Overview: -Universities typically have a course where computer science students learn to work with registers and very low level coding. This simulator allows for a simple and effective machine language environment. This works with a language called BasicML, which consists of instructions written in a signed four-digit decimal number, such as +1050, -5768, etc. This simulator has a 100-word memory, which are referenced by the last two digits of whatever command is entered. The BasicML program must be loaded into main memory starting at location 00 before running the program. The simulator contains a CPU, register, and main memory. There is also an accumulator, which is what we will use to do different arithmetic or move around numbers in various ways. Each instruction (signed four-digit decimal number) occupies one word of the 100 word memory. Each slot may contain one of these instructions, or a data value that is placed inside one of the memory slots to be worked with.
            The first two digits of each instruction are the operation code specifying the operation to be performed.
